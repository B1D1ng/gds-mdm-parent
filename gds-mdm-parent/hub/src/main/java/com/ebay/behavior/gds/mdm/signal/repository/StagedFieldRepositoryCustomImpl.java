...
public class StagedFieldRepositoryCustomImpl implements StagedFieldRepositoryCustom {

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Set<SignalChildId> findFieldIdsBySignalIds(Set<VersionedId> signalIds) {
        if (signalIds.isEmpty()) {
            return Set.of();
        }

        // Construct the query dynamically since JQL does not support IN clause with composite keys
        var queryStr = "SELECT new com.ebay.behavior.gds.mdm.signal.model.SignalChildId(s.signal.id, s.signal.version, s.id) "
                + "FROM StagedField s WHERE ";
