...
class StagedSignalStagingViewRepositoryIT {

    @Autowired
    private StagedSignalRepository signalRepository;

    @Autowired
    private StagedSignalStagingViewRepository viewRepository;
    ...
    void setUpClass() {
        ...
        var signal1 = stagedSignal(planId).toBuilder()
                .id(id).name("name1").dataSource(QA).platformId(CJS_PLATFORM_ID).environment(PRODUCTION).version(1).build(); // first version
        var signal2 = stagedSignal(planId).toBuilder()
                .id(id).name("name2").dataSource(QA).platformId(CJS_PLATFORM_ID).environment(PRODUCTION).version(2).build(); // second version
        signal3 = stagedSignal(planId).toBuilder()
                .id(id).name("name3").dataSource(QA).platformId(CJS_PLATFORM_ID).environment(STAGING).version(3).build(); // latest version
    ...
    void findAllByDataSourceAndPlatform() {
        ...
        assertThat(signals.get(0).getId()).isEqualTo(signal3.getId());
        assertThat(signals.get(0).getVersion()).isEqualTo(signal3.getVersion());
        assertThat(signals.get(0).getEnvironment()).isEqualTo(STAGING);
    ...
    void findAll() {
        ...
        assertThat(signals).extracting("id").contains(signal3.getId(), signal4.getId());
        assertThat(signals).extracting("version").contains(signal3.getVersion(), signal4.getVersion());
    }
}
