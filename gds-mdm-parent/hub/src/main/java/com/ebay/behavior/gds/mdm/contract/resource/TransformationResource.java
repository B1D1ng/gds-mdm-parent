...
@Validated
@Path(V1 + CMM + DEFINITION + "/transformation")
@RestController
...
public class TransformationResource {
    ...
    @POST
    @Operation(summary = "Create new Transformation")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created the Transformation",
                    content = {@Content(schema = @Schema(implementation = Transformation.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response create(Transformation transformation) {
        validateModerator(getRequestUser(), configuration);
        val persist = service.create(transformation);
        return created(uriInfo, persist);
    }

    @GET
    @Path("/{id}")
    @Operation(summary = "Get Transformation metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get Transformation metadata",
                    content = {@Content(schema = @Schema(implementation = Transformation.class))}),
    ...
    @PATCH
    @Path("/{id}")
    @Operation(summary = "Update existing Transformation")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the Transformation",
                    content = {@Content(schema = @Schema(implementation = Transformation.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
    ...
    public Response update(@PathParam(ID) Long id, Transformation transformation) {
        validateModerator(getRequestUser(), configuration);
        validateUpdateRequestId(transformation, id);
        val persisted = service.update(transformation);
        return Response.ok(persisted).build();
    }

    @DELETE
    @Path("/{id}")
    @Operation(summary = "Delete Transformation metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Deleted Transformation metadata",
                    content = {@Content(schema = @Schema(implementation = Transformation.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
