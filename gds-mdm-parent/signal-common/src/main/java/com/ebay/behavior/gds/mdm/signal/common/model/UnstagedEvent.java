...
@Entity
@Table(name = "event")
public class UnstagedEvent extends MetadataEvent implements Event, Metadata {
    ...
    @CollectionTable(name = "event_page_map", joinColumns = @JoinColumn(name = "event_id"))
    @Column(name = "page_id")
    private Set<Long> pageIds;

    @DiffInclude
    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "event_module_map", joinColumns = @JoinColumn(name = "event_id"))
    @Column(name = "module_id")
    private Set<Long> moduleIds;

    @DiffInclude
    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "event_click_map", joinColumns = @JoinColumn(name = "event_id"))
