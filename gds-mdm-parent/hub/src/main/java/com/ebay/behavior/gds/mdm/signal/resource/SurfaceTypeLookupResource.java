...
@Validated
@Path(V1 + LOOKUP + "/surface-type")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class SurfaceTypeLookupResource {
    ...
    @POST
    @Operation(summary = "Create a new surface type")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create a new surfaceType"),
            @ApiResponse(responseCode = "400", description = "Bad request")
    })
    public Response create(SurfaceTypeLookup lookup) {
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update existing surface type")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the surfaceType",
                    content = {@Content(schema = @Schema(implementation = SurfaceTypeLookup.class))}),
    ...
    public Response update(@PathParam(ID) Long id, SurfaceTypeLookup lookup) {
    ...
    @GET
    @Operation(summary = "Search surface types")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all surface types",
                    content = {@Content(schema = @Schema(implementation = SurfaceTypeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get surface type by id",
                    content = {@Content(schema = @Schema(implementation = SurfaceTypeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
