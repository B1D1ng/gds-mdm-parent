...
import com.ebay.behavior.gds.mdm.contract.service.FilterService;
...
public class FilterResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created the Filter",
                    content = {@Content(schema = @Schema(implementation = Filter.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response create(Filter filter) {
        validateModerator(getRequestUser(), configuration);
        val persist = service.create(filter);
        return created(uriInfo, persist);
    }

    @GET
    @Path("/{id}")
    @Operation(summary = "Get Filter metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get Filter metadata",
                    content = {@Content(schema = @Schema(implementation = Filter.class))}),
    ...
    @PATCH
    @Path("/{id}")
    @Operation(summary = "Update existing Filter")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the Filter",
                    content = {@Content(schema = @Schema(implementation = Filter.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
    ...
    public Response update(@PathParam(ID) Long id, Filter filter) {
        validateModerator(getRequestUser(), configuration);
        validateUpdateRequestId(filter, id);
        val persisted = service.update(filter);
        return Response.ok(persisted).build();
    }

    @DELETE
    @Path("/{id}")
    @Operation(summary = "Delete Filter metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Deleted Filter metadata",
                    content = {@Content(schema = @Schema(implementation = Filter.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
