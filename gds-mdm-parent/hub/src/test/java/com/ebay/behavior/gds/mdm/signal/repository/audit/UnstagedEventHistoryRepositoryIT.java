...
import static com.ebay.behavior.gds.mdm.signal.testUtil.TestModelUtils.unstagedEventHistory;
import static org.assertj.core.api.Assertions.assertThat;

@ActiveProfiles(IT)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class UnstagedEventHistoryRepositoryIT {

    @Autowired
    private UnstagedEventHistoryRepository repository;

    private UnstagedEventHistory model;

    @BeforeEach
    void setUp() {
        model = unstagedEventHistory();
    }
    ...
    void findByOriginalId() {
        ...
        var model1 = unstagedEventHistory().withOriginalId(originalId);
        var model2 = unstagedEventHistory().withOriginalId(originalId).withOriginalRevision(1).withType(UPDATED);
