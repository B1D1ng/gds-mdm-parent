...
import static com.ebay.behavior.gds.mdm.signal.testUtil.TestModelUtils.eventTemplateHistory;
import static org.assertj.core.api.Assertions.assertThat;

@ActiveProfiles(IT)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class EventTemplateHistoryRepositoryIT {

    @Autowired
    private EventTemplateHistoryRepository repository;

    private EventTemplateHistory model;

    @BeforeEach
    void setUp() {
        model = eventTemplateHistory();
    }
    ...
    void findByOriginalId() {
        ...
        var model1 = eventTemplateHistory().withOriginalId(originalId);
        var model2 = eventTemplateHistory().withOriginalId(originalId).withOriginalRevision(1).withType(UPDATED);
