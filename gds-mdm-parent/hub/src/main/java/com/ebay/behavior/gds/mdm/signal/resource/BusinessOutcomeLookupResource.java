...
import com.ebay.behavior.gds.mdm.signal.service.BusinessOutcomeLookupService;
...
@Validated
@Path(V1 + LOOKUP + "/business-outcome")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class BusinessOutcomeLookupResource {
    ...
    @POST
    @Operation(summary = "Create a new business outcome action")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create a new business outcome action"),
            @ApiResponse(responseCode = "400", description = "Bad request")
    })
    public Response create(BusinessOutcomeLookup lookup) {
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update existing business outcome action")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the business outcome action",
                    content = {@Content(schema = @Schema(implementation = BusinessOutcomeLookup.class))}),
    ...
    public Response update(@PathParam(ID) Long id, BusinessOutcomeLookup lookup) {
    ...
    @GET
    @Operation(summary = "Search business outcome actions")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all business outcome actions",
                    content = {@Content(schema = @Schema(implementation = BusinessOutcomeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @GET
    @Path("/{id}")
    @Operation(summary = "Get business outcome action by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get business outcome action by id",
                    content = {@Content(schema = @Schema(implementation = BusinessOutcomeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
