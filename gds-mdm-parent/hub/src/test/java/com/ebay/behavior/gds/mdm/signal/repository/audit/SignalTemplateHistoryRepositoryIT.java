...
import static com.ebay.behavior.gds.mdm.signal.testUtil.TestModelUtils.signalTemplateHistory;
import static org.assertj.core.api.Assertions.assertThat;

@ActiveProfiles(IT)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class SignalTemplateHistoryRepositoryIT {

    @Autowired
    private SignalTemplateHistoryRepository repository;

    private SignalTemplateHistory model;

    @BeforeEach
    void setUp() {
        model = signalTemplateHistory();
    }
    ...
    void findByOriginalId() {
        ...
        var model1 = signalTemplateHistory().withOriginalId(originalId);
        var model2 = signalTemplateHistory().withOriginalId(originalId).withOriginalRevision(1).withType(UPDATED);
        ...
        assertThat(found.get(0).getCreateDate()).isEqualTo(model1.getCreateDate());
        assertThat(found.get(0).getUpdateDate()).isEqualTo(model1.getUpdateDate());
        ...
        assertThat(found.get(1).getCreateDate()).isEqualTo(model2.getCreateDate());
        assertThat(found.get(1).getUpdateDate()).isEqualTo(model2.getUpdateDate());
