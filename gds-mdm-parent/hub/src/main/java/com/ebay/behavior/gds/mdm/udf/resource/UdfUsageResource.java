...
import com.ebay.behavior.gds.mdm.udf.service.UdfUsageService;
...
@Validated
@Path(V1 + UDFMM + "/udf-usage")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class UdfUsageResource {

    @Context
    private UriInfo uriInfo;

    @Autowired
    private UdfUsageService service;

    @POST
    @Operation(summary = "Create new Udf Usage")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created the udf usage", content = {@Content(schema = @Schema(implementation = UdfUsage.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response create(UdfUsage udfUsage) {
        val persisted = service.create(udfUsage);
