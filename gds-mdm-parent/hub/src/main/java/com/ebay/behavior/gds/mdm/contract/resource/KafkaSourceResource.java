...
@Validated
@Path(V1 + CMM + DEFINITION + "/kafka-source")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class KafkaSourceResource {

    @Autowired
    private KafkaSourceService service;
    ...
    @POST
    @Operation(summary = "Create new Kafka Source")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created the Kafka Source",
                    content = {@Content(schema = @Schema(implementation = KafkaSource.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response create(KafkaSource kafkaSource) {
        validateModerator(getRequestUser(), configuration);
        val persisted = service.create(kafkaSource);
        return created(uriInfo, persisted);
    }

    @GET
    @Path("/{id}")
    @Operation(summary = "Get Kafka Source metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get Kafka Source metadata",
                    content = {@Content(schema = @Schema(implementation = KafkaSource.class))}),
    ...
    @PATCH
    @Path("/{id}")
    @Operation(summary = "Update existing Kafka Source")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the Kafka Source",
                    content = {@Content(schema = @Schema(implementation = KafkaSource.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
    ...
    public Response update(@PathParam(ID) Long id, KafkaSource kafkaSource) {
        validateModerator(getRequestUser(), configuration);
        validateUpdateRequestId(kafkaSource, id);
        val updated = service.update(kafkaSource);
        return Response.ok(updated).build();
    }

    @DELETE
    @Path("/{id}")
    @Operation(summary = "Delete Kafka Source metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Deleted Kafka Source metadata",
                    content = {@Content(schema = @Schema(implementation = KafkaSource.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @PUT
    @Operation(summary = "Search for kafka source based on search query specification")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Search for kafka source based on search query specification",
                    content = {@Content(schema = @Schema(implementation = KafkaSource.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
