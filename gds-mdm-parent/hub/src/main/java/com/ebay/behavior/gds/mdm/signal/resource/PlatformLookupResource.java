...
@Validated
@Path(V1 + LOOKUP + "/platform")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class PlatformLookupResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create a new platform"),
            @ApiResponse(responseCode = "400", description = "Bad request")
    })
    public Response create(PlatformLookup lookup) {
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update existing platform")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the platform",
                    content = {@Content(schema = @Schema(implementation = PlatformLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response update(@PathParam(ID) Long id, PlatformLookup lookup) {
    ...
    @GET
    @Operation(summary = "Search platform")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all platform",
                    content = {@Content(schema = @Schema(implementation = PlatformLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get platform by id",
                    content = {@Content(schema = @Schema(implementation = PlatformLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
