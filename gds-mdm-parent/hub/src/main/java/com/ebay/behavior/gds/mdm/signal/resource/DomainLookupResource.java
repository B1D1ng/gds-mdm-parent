...
@Validated
@Path(V1 + LOOKUP + "/domain")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class DomainLookupResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create a new domain"),
            @ApiResponse(responseCode = "400", description = "Bad request")
    })
    public Response create(SignalDimValueLookup lookup) {
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update existing domain")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the domain",
                    content = {@Content(schema = @Schema(implementation = SignalDimValueLookup.class))}),
    ...
    public Response update(@PathParam(ID) Long id, SignalDimValueLookup lookup) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all Domains",
                    content = {@Content(schema = @Schema(implementation = SignalDimValueLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get Domain by id",
                    content = {@Content(schema = @Schema(implementation = SignalDimValueLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
