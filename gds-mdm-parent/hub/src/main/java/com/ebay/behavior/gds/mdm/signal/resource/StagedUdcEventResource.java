...
@Validated
@Path(V1 + METADATA + ELASTICSEARCH + "/event")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class StagedUdcEventResource {

    @Autowired
    private StagedUdcEventService service;
    ...
    @GET
    @Path("/{id}")
    @Operation(summary = "Get a staged Event by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get staged Event by id",
                    content = {@Content(schema = @Schema(implementation = UnstagedEvent.class))}),
            @ApiResponse(responseCode = "404", description = "staged Event not found",
    ...
    @GET
    @Operation(summary = "Get all staged Events")
    @ApiResponses(value = {
    ...
    @PUT
    @Path("/search")
    @Operation(summary = "Search for staged Events based on elasticsearch query")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all Events",
                    content = {@Content(schema = @Schema(implementation = Page.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
