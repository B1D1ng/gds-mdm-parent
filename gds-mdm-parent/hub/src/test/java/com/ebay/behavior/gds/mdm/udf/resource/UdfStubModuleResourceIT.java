...
class UdfStubModuleResourceIT extends AbstractResourceTest {

    private UdfStubModule udfStubModule;

    @BeforeEach
    void setUp() {
        url = getBaseUrl() + V1 + UDFMM + "/udf/stub/module";
        udfStubModule = TestModelUtils.udfStubModule();
    }

    @Test
    void create() {
        var created = requestSpecWithBody(udfStubModule)
                .when().post(url)
                .then().statusCode(HttpStatus.CREATED.value())
                .extract().body().jsonPath().getObject(".", UdfStubModule.class);

        assertThat(created.getId()).isNotNull();
        assertThat(created.getUdfStubModuleName()).isEqualTo("tracking-lib-spark");
    }
}
