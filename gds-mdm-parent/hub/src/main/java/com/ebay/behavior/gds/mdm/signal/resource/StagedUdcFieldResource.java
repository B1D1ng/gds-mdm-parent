...
@Validated
@Path(V1 + METADATA + ELASTICSEARCH + "/field")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class StagedUdcFieldResource {

    @Autowired
    private StagedUdcFieldService service;
    ...
    @GET
    @Path("/{id}")
    @Operation(summary = "Get a staged Field by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get staged Field by id",
                    content = {@Content(schema = @Schema(implementation = UnstagedField.class))}),
            @ApiResponse(responseCode = "404", description = "staged Field not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response getById(@PathParam(ID) Long id) {
        val field = service.getById(id);
        return Response.ok(field).build();
    }

    @GET
    @Operation(summary = "Get all staged Fields")
    @ApiResponses(value = {
    ...
    @PUT
    @Path("/search")
    @Operation(summary = "Search for staged Fields based on elasticsearch query")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all Fields",
                    content = {@Content(schema = @Schema(implementation = Page.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
