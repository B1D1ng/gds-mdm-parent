...
public class ClickResource {
    ...
    @GET
    @Path("/{id}")
    @Operation(summary = "Get pmsvc Click by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get pmsvc Click by id"),
            @ApiResponse(responseCode = "417", description = "Id not found")
    })
    public Response getById(@PathParam(ID) long id) {
        return Response.ok(service.getClickById(id)).build();
    }

    @GET
    @Operation(summary = "Get pmsvc Clicks by a list of ids")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get pmsvc Clicks by a list of ids",
    ...
    public Response getByIds(@NotNull @QueryParam("ids") String csvIds) {
        ...
        val persisted = service.getClickByIds(ids);
        return Response.ok(persisted).build();
    }

    @GET
    @Path("/search")
    @Operation(summary = "Get all pmsvc Clicks based on a search criterion")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all pmsvc Clicks"),
    ...
    public Response getAll(@NotNull @QueryParam(SEARCH_TERM) String searchTerm,
        ...
        return Response.ok(service.searchClicks(searchTerm, searchBy, searchCriterion)).build();
    }
}
