...
import static com.ebay.behavior.gds.mdm.signal.testUtil.TestModelUtils.unstagedSignalHistory;
import static org.assertj.core.api.Assertions.assertThat;

@ActiveProfiles(IT)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
class UnstagedSignalHistoryRepositoryIT {

    @Autowired
    private UnstagedSignalHistoryRepository repository;

    private UnstagedSignalHistory model;

    @BeforeEach
    void setUp() {
        model = unstagedSignalHistory();
    }
    ...
    void findByOriginalId() {
        ...
        var model1 = unstagedSignalHistory().withOriginalId(originalId);
        var model2 = unstagedSignalHistory().withOriginalId(originalId).withOriginalRevision(1).withType(UPDATED);
