...
class ClickResourceIT extends AbstractResourceTest {
    ...
    @BeforeEach
    void setUp() {
        url = getBaseUrl() + V1 + "/metadata/click";
    }
    ...
    void getById() {
        var persisted = requestSpec().when().get(url + '/' + id)
                ...
                .extract().body().jsonPath().getObject(".", ClickV1.class);
    ...
    void getByIds() {
        var models = requestSpec()
                ...
                .when().get(url)
                .then().statusCode(OK.value())
                .extract().body().jsonPath().getList(".", ClickV1.class);
