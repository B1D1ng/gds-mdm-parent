...
public class ModuleResource {
    ...
    @GET
    @Path("/{id}")
    @Operation(summary = "Get pmsvc Module by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get pmsvc Module by id"),
            @ApiResponse(responseCode = "417", description = "Id not found")
    })
    public Response getById(@PathParam(ID) long id) {
        return Response.ok(service.getModuleById(id)).build();
    }

    @GET
    @Operation(summary = "Get pmsvc Modules by a list of ids")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get pmsvc Modules by a list of ids",
    ...
    public Response getByIds(@NotNull @QueryParam("ids") String csvIds) {
        ...
        val persisted = service.getModuleByIds(ids);
        return Response.ok(persisted).build();
    }

    @GET
    @Path("/search")
    @Operation(summary = "Get all pmsvc Modules based on a search criterion")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all pmsvc Modules"),
    ...
    public Response getAll(@NotNull @QueryParam(SEARCH_TERM) String searchTerm,
        ...
        return Response.ok(service.searchModules(searchTerm, searchBy, searchCriterion)).build();
    }
}