...
@Validated
@Path(V1 + CMM + DEFINITION + "/kafka-sink")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class KafkaSinkResource {

    @Autowired
    private KafkaSinkService service;
    ...
    @POST
    @Operation(summary = "Create new Kafka Sink")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created the Kafka Sink",
                    content = {@Content(schema = @Schema(implementation = KafkaSink.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    public Response create(KafkaSink kafkaSink) {
        validateModerator(getRequestUser(), configuration);
        val persisted = service.create(kafkaSink);
        return created(uriInfo, persisted);
    }

    @GET
    @Path("/{id}")
    @Operation(summary = "Get Kafka Sink metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get Kafka Sink metadata",
                    content = {@Content(schema = @Schema(implementation = KafkaSink.class))}),
    ...
    @PATCH
    @Path("/{id}")
    @Operation(summary = "Update existing Kafka Sink")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the Kafka Sink",
                    content = {@Content(schema = @Schema(implementation = KafkaSink.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
    ...
    public Response update(@PathParam(ID) Long id, KafkaSink kafkaSink) {
        validateModerator(getRequestUser(), configuration);
        validateUpdateRequestId(kafkaSink, id);
        val persisted = service.update(kafkaSink);
        return Response.ok(persisted).build();
    }

    @DELETE
    @Path("/{id}")
    @Operation(summary = "Delete Kafka Sink metadata by id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Deleted Kafka Sink metadata",
                    content = {@Content(schema = @Schema(implementation = KafkaSink.class))}),
            @ApiResponse(responseCode = "417", description = "Id not found",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @PUT
    @Operation(summary = "Search for kafka sink based on search query specification")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Search for kafka sink based on search query specification",
                    content = {@Content(schema = @Schema(implementation = KafkaSink.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request",
                    content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
