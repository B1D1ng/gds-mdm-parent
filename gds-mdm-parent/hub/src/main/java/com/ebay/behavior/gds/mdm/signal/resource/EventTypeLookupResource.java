...
@Validated
@Path(V1 + LOOKUP + "/event-type")
@RestController
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class EventTypeLookupResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create a new eventType"),
            @ApiResponse(responseCode = "400", description = "Bad request")
    })
    public Response create(EventTypeLookup lookup) {
    ...
    @PUT
    @Path("/{id}")
    @Operation(summary = "Update existing event type")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Updated the eventType",
                    content = {@Content(schema = @Schema(implementation = EventTypeLookup.class))}),
    ...
    public Response update(@PathParam(ID) Long id, EventTypeLookup lookup) {
    ...
    @GET
    @Operation(summary = "Search event types")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get all event types",
                    content = {@Content(schema = @Schema(implementation = EventTypeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Get event type by id",
                    content = {@Content(schema = @Schema(implementation = EventTypeLookup.class))}),
            @ApiResponse(responseCode = "400", description = "Bad request", content = {@Content(schema = @Schema(implementation = ErrorMessageV3.class))})
    })
