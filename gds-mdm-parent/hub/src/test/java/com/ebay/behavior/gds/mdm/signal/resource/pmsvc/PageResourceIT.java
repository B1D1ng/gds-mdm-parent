...
class PageResourceIT extends AbstractResourceTest {

    private final long id = 2_481_888;
    private String url;

    @BeforeEach
    void setUp() {
        url = getBaseUrl() + V1 + "/metadata/page";
    }
    ...
    void getById() {
        var persisted = requestSpec().when().get(url + '/' + id)
                ...
                .extract().body().jsonPath().getObject(".", PageV1.class);
    ...
    void getByIds() {
        var models = requestSpec()
                ...
                .when().get(url)
                .then().statusCode(OK.value())
                .extract().body().jsonPath().getList(".", PageV1.class);
