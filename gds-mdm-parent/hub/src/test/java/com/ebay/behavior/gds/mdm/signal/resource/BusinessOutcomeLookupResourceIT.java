...
class BusinessOutcomeLookupResourceIT extends AbstractResourceTest {
    ...
    @BeforeEach
    void setUp() {
        url = getBaseUrl() + V1 + LOOKUP + "/business-outcome/";
        lookup = service.getByName(businessOutcome().getName());
    }

    @Test
    void create_badId_error() {
    ...
    @Test
    void create() {
        service.deleteByName(lookup.getName());
        var created = requestSpecWithBody(businessOutcome())
                .when().post(url)
                .then().statusCode(HttpStatus.CREATED.value())
                .extract().body().jsonPath().getObject(".", BusinessOutcomeLookup.class);
    ...
    void getAll() {
        var domains = requestSpec()
                .when().get(url)
                .then().statusCode(HttpStatus.OK.value())
                .extract().body().jsonPath().getList(".", BusinessOutcomeLookup.class);
